...
$(document).ready(function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.flag) {
					...
					debugger;
					alert(data.result);
					if (data.result.changYueDay.length == 0 &&
							data.result.changYueDaySum.length == 0){
					//	$("#changYue").hide();
						alert("未查询到有效数据!");
					}else{
						var changYue = data.result.changYueDay;
						var changYue2 = data.result.changYueDaySum;
	...
	function chartScene1(result,result1) {
		var myFileChart = echarts.init(document.getElementById('chartScene1'));
		myFileChart.clear();
		// 显示标题，图例和空的坐标轴
